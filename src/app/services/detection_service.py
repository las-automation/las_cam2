...
class DetectionService:
    ...
    # --- MÉTODO _get_best_backend (SINTAXE CORRIGIDA) ---
    def _get_best_backend(self) -> None:
        ...
        def try_set_backend(name: str, model_path: str, device_args: dict, check_path: bool = True) -> bool:
            ...
            self.selected_model_path = str(model_p)
        ...
        if preference != "auto":
            ...
            if preference == "tensorrt" and torch.cuda.is_available():
                ...
                try:
                    ...
                    if torch_directml.is_available():
                        # DirectML usa modelo .pt e NENHUM device_arg (para evitar o ValueError)
            ...
            print(
                f"   ⚠️ Preferência '{preference}' falhou ou hardware/modelo incompatível. Tentando detecção automática...")
        ...
        if torch.cuda.is_available() and try_set_backend("TensorRT", cfg.model_path_tensorrt, {'device': 0}):
            ...
            try:
                ...
                pass
            return  # <-- CORRIGIDO: return está FORA do try interno
        # DirectML
        try:
            import torch_directml
        ...
        if try_set_backend("CPU", cfg.model_path, {'device': 'cpu'}):
            ...
            if preference == 'auto': print(
                "      💡 Para melhor performance, considere instalar dependências de aceleração.")
            return
        self.backend_name = "N/A"
        print(f"   ❌ Nenhum backend de detecção pôde ser configurado!")

    # --- FIM _get_best_backend ---
    ...
    def start_detection(
        ...
        session = DetectionSession(camera_id=camera_id, user=username, model_version=self.backend_name,
                                   cargo_type=cargo_type)
        stop_event = threading.Event()
        thread = threading.Thread(target=self._run_detection_thread,
                                  args=(camera_id, session, camera_config, stop_event, callback), daemon=True,
                                  name=f"Detection-Cam-{camera_id}")
        ...
        thread.start()
        log_user_action(username,
                        f"DETECTION_STARTED_CAMERA_{camera_id}_TYPE_{cargo_type.value}_BACKEND_{self.backend_name}")
        return True
    ...
    def _run_detection_thread(
        ...
        try:
            ...
            # --- CORREÇÃO: Usa 'source' e trata webcam/RTSP ---
            ...
            while not stop_event.is_set():
                ...
                # --- LÓGICA DE LINHA E CONTAGEM (CORRETA) ---
                ...
                if deteccoes is not None and deteccoes.id is not None:
                    ...
                    for box, obj_id_tensor in zip(deteccoes.xyxy, deteccoes.id):
                        ...
                        if height > 0:
                            ...
                            if obj_id not in rastreador_estado: rastreador_estado[obj_id] = {
                                'previous_fraction_below': None, 'counted_this_crossing_up': False}
                            ...
                            if (previous_fraction_below is not None and
                                    ...
                                    dentro_limites_x):
                                contador += 1;
                                state['counted_this_crossing_up'] = True;
                ...
                # --- FIM LÓGICA DE LINHA E CONTAGEM ---
    ...
    # --- MÉTODO stop_detection (CORRIGIDO) ---
    def stop_detection(self, camera_id: int) -> bool:
        """Para detecção em uma câmera."""
        # Verifica se há algo para parar (evento OU thread)
        if camera_id not in self._stop_events and camera_id not in self._detection_threads:
            # log_system_event(f"STOP_DETECTION_IGNORED: Camera {camera_id} já parada", camera_id)
            return True  # Considera sucesso se já está parada

        log_system_event(f"STOPPING_DETECTION_REQUESTED: Camera ID: {camera_id}", camera_id);
        print(f"⏳ [{threading.current_thread().name}] Solicitando parada da Câmera {camera_id}...")
        ...
        if thread and thread.is_alive():
            print(f"   Aguardando thread {thread.name} finalizar...");
            thread.join(timeout=7.0)
            if thread.is_alive():
                log_error("DetectionService", None, f"Thread {thread.name} não finalizou no timeout!")
            else:
                stopped_cleanly = True;
                print(f"   Thread {thread.name} finalizada.")
        else:
            stopped_cleanly = True  # Thread já não estava ativa

        # --- Limpeza de referências ---
        # NÃO remove a sessão (_active_sessions.pop), o Controller precisa dela
        self._stop_events.pop(camera_id, None)
        self._detection_threads.pop(camera_id, None)
        # --- FIM LIMPEZA ---

        log_system_event(f"DETECTION_STOPPED_CONFIRMED: Camera ID: {camera_id}", camera_id);
        print(f"🛑 [{threading.current_thread().name}] Detecção da Câmera {camera_id} confirmada como parada.")

        self.trigger_ui_event("detection_stopped", camera_id)  # Notifica UI que parou
        return stopped_cleanly

    # --- FIM stop_detection ---

    def stop_all_detections(self) -> None:
        """Para todas as detecções ativas"""
        ...
        log_system_event(f"STOPPING_ALL_DETECTIONS: Cameras {camera_ids}");
        print(f"⏳ [{threading.current_thread().name}] Solicitando parada de todas as {len(camera_ids)} detecções...")
        threads_to_join = []
        for camera_id in camera_ids:
            stop_event = self._stop_events.get(camera_id);
            if stop_event: stop_event.set()
            thread = self._detection_threads.get(camera_id)
            if thread and thread.is_alive(): threads_to_join.append((thread, camera_id))  # Guarda ID
        print(f"   Aguardando {len(threads_to_join)} threads finalizarem...");
        for thread, camera_id in threads_to_join:
            thread.join(timeout=7.0)
            if thread.is_alive(): log_error("DetectionService", None,
                                            f"Thread {thread.name} (Cam {camera_id}) não finalizou no timeout durante stop_all!")
            # Limpa refs de thread/evento
            self._stop_events.pop(camera_id, None)
            self._detection_threads.pop(camera_id, None)

        # Limpa sessões ativas restantes (se houver)
        self._active_sessions.clear()

        print(f"   Threads finalizadas (ou timeout).")
        log_system_event("ALL_DETECTIONS_STOPPED_CONFIRMED");
        print(f"🛑 [{threading.current_thread().name}] Todas as detecções confirmadas como paradas.")

    def is_detection_active(self, camera_id: int) -> bool:
        """Verifica se a thread de detecção está ativa para uma câmera"""
        thread = self._detection_threads.get(camera_id);
    ...
    def get_detection_count(self, camera_id: int) -> int:
        ...
        return session.detection_count if session else 0

    def reset_count(self, camera_id: int) -> bool:
        """Reseta contagem de uma câmera (na sessão)."""
        session = self._active_sessions.get(camera_id)
        if session:
            session.detection_count = 0
            # TODO: Notificar a thread ativa para limpar 'rastreador_estado'
            print(
                f"⚠️ [{threading.current_thread().name}] Contagem resetada para Cam {camera_id}, mas estado interno da thread pode não ter sido limpo.")
            log_system_event(f"COUNT_RESET_CAMERA_{camera_id}", camera_id)
            self.trigger_ui_event("count_reset", camera_id)  # Notifica UI
            return True
        log_error("DetectionService", None, f"Tentativa de resetar contagem para câmera inativa: {camera_id}");
        return False

    def get_session(self, camera_id: int) -> Optional[DetectionSession]:
        """Retorna sessão (pode estar ativa ou recém-finalizada)."""
        return self._active_sessions.get(camera_id)

    def get_camera_status(self, camera_id: int) -> Optional[CameraStatus]:
        """Retorna status de uma câmera."""
        is_active = self.is_detection_active(camera_id)
        session = self._active_sessions.get(camera_id)
        thread_exists = camera_id in self._detection_threads

        if not thread_exists and not session: return None
        if not is_active and not session and camera_id not in self._detection_threads: return None  # Limpo pelo stop

        count = session.detection_count if session else 0
        start = session.start_time if session else None

        return CameraStatus(camera_id=camera_id, is_active=is_active, detection_count=count, session_start=start,
                            backend=self.backend_name)

    def get_backend_info(self) -> dict:
        """Retorna informações sobre o backend em uso e sessões ativas."""
        active_count = sum(1 for cam_id in self._detection_threads if self.is_detection_active(cam_id))
        return {'backend_name': self.backend_name, 'model_path': self.selected_model_path,
                'device_args': self.selected_device_args, 'active_sessions': active_count}